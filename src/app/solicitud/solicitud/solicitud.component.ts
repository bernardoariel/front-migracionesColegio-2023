...
export class SolicitudComponent implements OnInit {
  ...
  seleccionarAutorizante1(autorizante: IPersona) {
    ...
    modalPersona.afterClosed().subscribe((autorizante: IPersona) => {
      this.ListaAutorizantes.cargarMenores();
    });
  }
  ...
  seleccionarAcompaneante(acompaneante: IPersona) {
    ...
    modalAcompaneante.afterClosed().subscribe((acompanenante: IPersona) => {
      this.ListaAcompaneantes.cargarMenores();
    });
  }

  eliminarMenor(index: number): void {
    this.solicitudService.eliminarCampo(this.solicitud, 'menores');
